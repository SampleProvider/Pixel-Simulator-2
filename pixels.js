...
var colorLerp = function(r1, g1, b1, r2, g2, b2, p) {
    return [
        (r1 * (Math.sin(gameTick * Math.PI / p) + 1) / 2) + (r2 * (Math.sin((gameTick + p) * Math.PI / p) + 1) / 2),
        (g1 * (Math.sin(gameTick * Math.PI / p) + 1) / 2) + (g2 * (Math.sin((gameTick + p) * Math.PI / p) + 1) / 2),
        (b1 * (Math.sin(gameTick * Math.PI / p) + 1) / 2) + (b2 * (Math.sin((gameTick + p) * Math.PI / p) + 1) / 2),
    ];
};
...
var pixels = {
    ...
    "sand": {
        ...
        update: function(x, y) {
            if (getTouchingDiagonal(x, y, "sand", null) == 8) {
                return;
            }
    ...
    "water": {
        draw: function(x, y) {
            fill(125, 225, 255);
            rect(x * pixelSize, y * pixelSize, pixelSize, pixelSize);
            fill(25, 75, 175, noise(x / 10, y / 10, 0) * 255);
            rect(x * pixelSize, y * pixelSize, pixelSize, pixelSize);
        },
        drawPreview: function(ctx) {
        ...
        update: function(x, y) {
            if (getTouchingDiagonal(x, y, "water", null) == 8) {
                return;
            }
            flow(x, y, gridSize);
        },
    ...
    "lava": {
        draw: function(x, y) {
            fill(255, 25, 0);
            rect(x * pixelSize, y * pixelSize, pixelSize, pixelSize);
            fill(225, 255, 25, noise(x / 10, y / 10, 0) * 255);
            rect(x * pixelSize, y * pixelSize, pixelSize, pixelSize);
        },
        drawPreview: function(ctx) {
        ...
        update: function(x, y) {
            ...
            forAllTouching(x, y, function(x1, y1) {
                ...
                if (grid[y1][x1][1] != "fire" && grid[y1][x1][0] != "lava" && random() < 0.1) {
                    // var canChange = true;
    ...
    "silt": {
        ...
        update: function(x, y) {
            if (isTouchingDiagonal(x, y, "lava", null) && !isTouchingDiagonal(x, y, "water", null) && random() < 0.1) {
    ...
    "leaf": {
        ...
        blast_resistance: 1,
        name: "Leaf",
        description: "A nice, springy leaf. Drops seeds when it decays.",
        key: 32,
    },
    "seed": {
        draw: function(x, y) {
    ...
    "corruption": {
        ...
        update: function(x, y) {
            ...
            forAllTouching(x, y, function(x1, y1) {
                if (random() < 0.1 / pixels[grid[y1][x1][0]].blast_resistance && grid[y1][x1][0] != "air") {
...
var update = function() {
    ...
    document.getElementById("fpsDisplay").innerHTML = "FPS: " + frames.length;
